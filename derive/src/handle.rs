...
pub(crate) fn derive_handle(input: &syn::DeriveInput) -> Result<TokenStream> {
    ...
    for attr in input
        .attrs
        .iter()
        .filter(|attr| attr.path().is_ident("handle"))
    {
    ...
    let Some(id) = id else {
        let err_msg = "Handle derive requires `#[handle(unsafe(id(...)))]`";
