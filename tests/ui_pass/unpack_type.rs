...
impl UnpackAs<u32, i32> for PairParts {
    fn into_parts(value: Self::CType) -> (u32, i32) {
        (value.0.into(), value.1.into())
    }
}

impl<C: ReprC> TryUnpackAs<*const C, i16> for &OdbcStr<C>
where
    Self: UnpackAs<*const C, usize>,
{
    type Error = core::num::TryFromIntError;

    fn try_into_parts(value: Self::CType) -> Result<(*const C, i16), Self::Error> {
        let (data, len) = <Self as UnpackAs<*const C, usize>>::into_parts(value);
...
ffi! {
    ...
    #[symbol_name = "unpack_len_impl"]
    fn unpack_len(#[unpack_as(_, _)] values: &[u32]) -> usize;

    #[symbol_name = "unpack_convert"]
    fn unpack_convert(#[unpack_as(u32, i32)] move value: PairParts) -> u32;

    #[symbol_name = "optional_slice_len_impl"]
    fn optional_slice_len(#[unpack_as(_, _)] values: Option<&[u32]>) -> usize;

    #[symbol_name = "optional_slice_mut_len_impl"]
    fn optional_slice_mut_len(#[try_unpack_as(_, usize)] values: Option<&mut [u32]>) -> usize;

    fn borrowed_box(#[unpack_as(*const u32, usize)] value: Box<[u32]>);
    fn moved_box(#[unpack_as(_, _)] move value: Box<[u32]>);
    fn optional_moved_box(#[unpack_as(_, _)] move value: Option<Box<[u32]>>);
    fn parenthesized_ref(#[unpack_as(_, _)] value: (&[u32]));
    fn parenthesized_tuple(#[unpack_as(_, _)] move value: ((u8, u16)));

    #[symbol_name = "static_unpack_{C}"]
    fn static_unpack<C>(
        #[try_unpack_as(_, _)] move value: (C, u8),
    )
    where
        use<C> @ (<u8> | <u16>);
    ...
    fn dispatch_associated_unpack<dyn(u8) T: TargetType>(
        #[try_unpack_as(*mut u8, usize)] value: Option<&mut <T as TargetType>::Value>,
    ) -> usize
    where
        use<T> @ <ByteTarget>;
    ...
    impl Counter {
        #[symbol_name = "inherent_unpack_len"]
        fn imported_inherent_unpack_len(&self, #[unpack_as(_, _)] values: &[u32]) -> usize;
    }

    impl ImportUnpackLen for Counter {
        #[symbol_name = "export_trait_unpack_len"]
        fn import_trait_unpack_len(&self, #[try_unpack_as(_, usize)] values: &[u32]) -> usize;
    }
}
...
ffi! {
    ...
    impl Statement {
        #[symbol_name = "static_method_try_unpack_as_dst_{C}"]
        fn prepare<C>(&self, #[try_unpack_as(_, i16)] text: &OdbcStr<C>)
        where
            use<C> @ (<u8> | <u16>);

        #[symbol_name = "static_method_try_unpack_as_dst_{C}_2"]
        fn prepare2<C>(&self, #[try_unpack_as(*const C, i16)] text: &OdbcStr<C>)
        where
            use<C> @ (<u8> | <u16>);

        #[symbol_name = "static_method_multiple_try_unpack_as_dst_{C}"]
        fn prepare_pair<C>(
            &self,
            #[try_unpack_as(_, i16)] first: &OdbcStr<C>,
            #[try_unpack_as(_, i16)] second: &OdbcStr<C>,
        )
        where
            use<C> @ (<u8> | <u16>);
    }
...
mod aliased_core_option {
    ...
    ffi! {
        ...
        fn aliased_optional_slice(#[unpack_as(_, _)] values: Option<&[u32]>);
    }
}
...
fn main() {
    ...
    let mut bytes = [1, 2, 3];
    assert_eq!(dispatch_associated_unpack::<ByteTarget>(Some(&mut bytes)), 3);
